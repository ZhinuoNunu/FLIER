...
class VisualTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, num_classes: int,
        ...
        self.head = nn.Linear(width, self.num_classes)  # (width, 1000)
...
class LinearHead(nn.Module):
    def __init__(self, in_feature: int, num_classes: int, batch_size: int):
        super().__init__()
        ...
        self.conv1 = torch.nn.Conv2d(4, 10, kernel_size=1)
        self.conv2 = torch.nn.Conv2d(10, 3, kernel_size=1)
        self.pooling = torch.nn.MaxPool2d(2)
        
        self.head = nn.Linear(self.in_feature, self.num_classes)  # (width, 1000)
        trunc_normal_(self.head.weight, std=.02)
...
@register_model
def CLIP_B32(pretrained=False, **kwargs):
    vision_width = 768
    vision_layers = 12
    vision_heads = vision_width // 64
    print(kwargs)
...
@register_model
def LINEAR_HEAD(pretrained=False, **kwargs):
    default_kwargs = {
        # 'output_dim': 512, from config
        'num_classes': kwargs['num_classes'],
        'in_feature': 768,
    ...
    model = LinearHead(**default_kwargs)
    return model
